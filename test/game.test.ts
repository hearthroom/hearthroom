...
const spec = { version: 1, npcs: [{ name: "阿罗娜", halo: "arc", pos: [0, -3] }], places: [{ match: "办公室", pos: [0, 4], label: "夏莱办公室" }] };
...
describe("遊戲模式配置", () => {
  ...
  it("形狀不對回 400 並列出錯在哪", async () => {
    const res = await put(MINE, { spec: { version: 1, npcs: [{ name: "x", pos: [1] }], places: [{ match: "(", pos: [0, 0], label: "a" }] } });
    expect(res.status).toBe(400);
    const body = (await res.json()) as { errors: string[] };
    expect(body.errors.join("\n")).toMatch(/npcs\[0\]\.pos/);
    expect(body.errors.join("\n")).toMatch(/places\[0\]\.match/);
  });
  ...
  it("作者存了之後任何人都讀得到；再存會覆蓋；刪掉就回 404", async () => {
    ...
    const body = (await read.json()) as { spec: { npcs: { name: string }[] } };
    expect(body.spec.npcs[0].name).toBe("阿罗娜");
    ...
    await put(MINE, { spec: { ...spec, npcs: [{ name: "早濑优香" }] } });
    const again = (await (await SELF.fetch(`https://c.test/v1/cards/${MINE}/game?_=2`)).json()) as { spec: { npcs: { name: string }[] } };
    expect(again.spec.npcs[0].name).toBe("早濑优香");
