 ...
 <div class="page badges-page">
  ...
  <template v-if="data">
   <section class="badge-curation panel">
    <div><h2>{{ t('badgeWall.featuredTitle') }}</h2><p class="subtle">{{ t('badgeWall.featureHint',{count:selected.length}) }}</p></div>
   ...
   <BadgeWall :items="items" :selected="selected" editable :busy="busy" @change="selected=$event;saved=false" />
