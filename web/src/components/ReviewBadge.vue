...
<template><span v-if="reviewer.reviewer && reviewer.pending > 0" class="review-badge" :class="{'review-badge--dot':dot}" :aria-label="$t('moderation.pending',{n:reviewer.pending})" :title="$t('moderation.pending',{n:reviewer.pending})">{{ dot ? '' : reviewer.pending > 99 ? '99+' : reviewer.pending }}</span></template>
<style scoped>
