...
import { fetchScoreRecords, fetchWallet, type Wallet, type ScoreRecord } from "@/lib/api";
import { clock, dateOnly, dayLabel, whole } from "@/lib/format";
import { accountToken } from "@/lib/connections";
import { can, apiBaseOf, providerName, type ProviderId } from "@/lib/provider";
    ...
    <div class="wallet__grid">
      <aside class="wallet__side">
        ...
        <section v-if="can('membership', provider)" class="panel plans">
          ...
          <ul v-if="wallet?.plans.length" class="plans__list">
            <li v-for="plan in wallet.plans" :key="plan.tier" class="plan">
