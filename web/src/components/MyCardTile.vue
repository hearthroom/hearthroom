...
<style scoped>
.card {
  display: grid; grid-template-columns: minmax(0, 2fr) minmax(0, 3fr); min-width: 0;
...
.card__art { position: relative; display: block; aspect-ratio: 9 / 16; align-self: start; border-radius: var(--r-sm); background: var(--surface-2); overflow: hidden; }
.card__art img { width: 100%; height: 100%; display: block; object-fit: cover; transition: transform var(--dur-slow) var(--ease); }

.card__art::after { content: ""; position: absolute; inset: -1px; box-shadow: inset 0 0 0 1px rgba(16, 16, 24, 0.05); pointer-events: none; }
...
.card__badge--muted { background: var(--surface-2); color: var(--text-2); }
.card__note { font-size: 12px; color: var(--danger); line-height: 1.5; }

.card__intro { display: flex; flex-direction: column; min-width: 0; gap: var(--s-2); }
.card__badges { display: flex; flex-wrap: wrap; gap: var(--s-1); }
.card__body { grid-column: 1 / -1; display: grid; min-width: 0; gap: var(--s-2); }
.card__name { font-size: 16px; font-weight: 600; line-height: 1.35; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; line-clamp: 2; -webkit-box-orient: vertical; overflow-wrap: anywhere; }
.card__hook {
...
.card__meta { margin-top: auto; font-size: 12px; color: var(--text-3); font-variant-numeric: tabular-nums; }
/* 工作區保留三個常用操作；長翻譯可換行，觸控高度不縮小。 */
.card__actions { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1.5fr); gap: var(--s-2); margin-top: 0; }
