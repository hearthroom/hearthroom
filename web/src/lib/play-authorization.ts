import { apiBaseOf, currentProvider } from './provider';
import { beginLogin } from './oauth';
/** Existing links remain valid; only the chat permission needs fresh consent. */
export async function ensurePlayAuthorization(token:string,returnTo:string):Promise<boolean> {
 ...
 if(response.status===403) {
  ...
  if(body.error==='insufficient_scope') {await beginLogin(returnTo,{provider});return false;}
 }
