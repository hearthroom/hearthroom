...
it('does not derive a download host from untrusted hosts',()=>{
 for(const host of ['localhost','sukisuki.ai.evil.test','evil.test'])expect(androidDownloadUrl(host)).toBe('https://downloads.hearthroom.club/latest.apk');
});
it.each(['zh-Hant','zh-Hans','en','ja','ko'])('offers the APK and installation instructions in %s without login',async locale=>{
 ...
 expect(el.querySelector('a[href="https://downloads.hearthroom.club/latest.apk"]')?.textContent).toContain(i18n.global.t('download.android'));
