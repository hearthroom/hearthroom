...
it('shows one work with direct actions and sends community review only to its original account',async()=>{await mount();expect(root.querySelectorAll('article.card')).toHaveLength(1);expect(root.textContent).toContain(fixture.summary);expect(root.querySelector('a[href*="single=1"]')).toBeNull();expect(root.querySelector('a[href*="100021/edit"]')?.getAttribute('href')).toContain('provider=harbor');button('mine.action.submit').click();await settle();expect(mocks.register).toHaveBeenCalledWith('original','token-harbor',false,[{provider:'lunatalk',token:'token-lunatalk'}],'harbor');expect(root.textContent).toContain(i18n.global.t('mine.badge.pending'));});
it('offers the actual copies for playtesting without navigation or publication on open',async()=>{await mount();button('mine.action.play').click();await settle();const choices=root.querySelectorAll('.play-choices a');expect(choices).toHaveLength(2);expect([...choices].map(a=>a.getAttribute('href'))).toEqual(expect.arrayContaining(['/play/100021?mode=source&provider=harbor','/play/100021?mode=source&provider=lunatalk']));expect(mocks.register).not.toHaveBeenCalled();});
