...
it('reloads before changing the host of an installed player',async()=>{const assign=vi.spyOn(window.location,'assign').mockImplementation(()=>{});const router=await mount('/play/100021?provider=harbor');await router.push('/play/100021?provider=lunatalk');await settle();expect(assign).toHaveBeenCalledWith('/play/100021?provider=lunatalk');expect(mocks.stage).toHaveBeenCalledTimes(1);assign.mockRestore();});
