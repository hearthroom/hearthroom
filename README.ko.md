...
- **번역** — UI 문자열은 `web/src/locales/<언어>.json`에 언어당 한 파일씩 있습니다. `npm run i18n -w web`으로 각 언어에 무엇이 빠졌는지 볼 수 있습니다. 새 문자열은 다섯 파일 모두에 추가해야 하며, 컴포넌트에 미번역 문자열이 남으면 빌드가 실패합니다. 카드 작성 가이드는 `docs/guide/`에 언어별로 한 파일씩 있습니다.
