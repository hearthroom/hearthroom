...
3. Open privacy settings. All switches start private/off except chat XP. Turn public badges on, visit the same public author page (including an author with no cards), and verify only the badge appears.
...
7. Open unlink confirmation: account and effects are visible, cancel has initial focus. Cancel makes no DELETE; confirm shows cleanup, hides case access and old case results become unauthorized.
