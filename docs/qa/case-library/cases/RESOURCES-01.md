...
14. Keep the collapsed provider overview compact: at 1440px its two rows show provider/storage and the complete copyable prefix. Expand formats/help using pointer and keyboard, then collapse without hiding the prefix. At 390px, require 44px overview/action targets, one-row English upload actions, no document overflow, and a visible first file row within the initial viewport.
