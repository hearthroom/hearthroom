...
- 對話清單由社群成員擁有，儲存在本站 `member_conversations`。播放器讀回已保存的玩家訊息或生成內容後，由 `conversationActivity` 事件登記服務、角色與續玩識別；只開卡、開場白及新空白對話不登記，也不覆蓋已玩過的紀錄；同一成員／服務／角色更新同一筆最近對話。列表只讀本站 `/v1/me/conversations`，不抓取任一家服務的 `/conversation/list`，也不自動匯入外部歷史。
