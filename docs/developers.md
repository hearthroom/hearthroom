...
| Error codes | The site translates these for end users: `invalid_arguments`, `role_in_review`, `visibility_requires_review`, `public_role_requires_clone`, `permission_denied`, `not_found`. Others pass through; a `message` that is not a code is shown verbatim (for example a moderation reason). |
