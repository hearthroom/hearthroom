...
審核頁不顯示作者（盲審）。過審綁著內容版本——作者之後改了卡，下一輪同步發現內容變了
就離榜、重新排隊。卡片離開榜單的另一種方式仍然是作者自己撤銷登記。

審核人要看得到作者的完整設定，而那是作者私有的。做法是主站的「逐卡授權」：作者提交
時，本站用作者自己的 token 替他把這張卡授權給本站在主站上的**審核機器人**（一個服務
帳號），審核人透過機器人讀設定。機器人只讀得到作者主動交出來的卡，主站那邊把它擋在
聊天、點數、建卡之外；作者隨時可以在主站收回。機器人的金鑰是本站持有的唯一秘密
（`wrangler secret put REVIEW_BOT_KEY`），不配它也能跑——那就回到「登記即上榜」。
...
| `POST /open/v1/share/role/grant` | 轉發作者自己的 token | 提交審核時，把卡授權給審核機器人 |
| `GET /open/v1/share/role/detail` | 審核機器人的金鑰 | 審核人打開審核頁 |
| `GET /open/v1/share/role/content-hash` | 審核機器人的金鑰 | 提交時記版本；每小時同步比對 |
