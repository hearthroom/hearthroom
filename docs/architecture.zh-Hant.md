...
接了兩家供應商之後的邊界（owner 2026-09-17）：**身分不混、榜單相通**。上游的卡片 ID 與作者數字 ID 在兩家各自編號會撞號，所以登記、「我的卡片」、審核都鎖在請求帶的那一家（`X-Provider`）；榜單、搜尋、標籤、作者榜則把兩家的卡列在同一個榜上，`X-Provider` 在那些路徑只代表「我用哪家的帳號」。一張卡搬到另一家的副本（`work_copies.role_id`）不重複上榜，卡片頁的「用哪家玩」才列副本；每小時同步把已發布副本的對話數加進來源那張。哪家能玩由 `src/providers.ts` 的 `HAS_CHAT` 決定——Harbor 是平台不做對話，它的卡列成「只存放」。搬運（`src/card-transfer.ts`）走兩家共用的契約路由，連世界書、作者資產、開場白備選一起搬；作者在「我的卡片」按登記時，站台在背景把卡同步到所有已綁定的渠道（登記即分發，不阻塞登記；結果落在 `work_copies`，分發面板可手動重送）。
...
**不保存生日**，只記驗過的時間。開了的請求帶 `?nsfw=1` 加 token，伺服器先驗身分再查，回應不進邊緣快取。
審核人要驗過年齡才能領成人內容的審核單。
