...
> 這份是站台各部分的設計筆記與取捨紀錄，從原本的 README 搬過來。給改程式碼的人看；一般介紹、使用方式與貢獻流程在倉庫根目錄的 [README](../README.md)。
...
**上榜前要經過社群自己的審核**（2026-09-07 起；在此之前是「登記即上榜」）。能不能真的
用這張卡仍然是上游決定的事；這裡審的是「這個社群要不要把它放上榜」。審核由社群成員
擔任的審核人負責：共享佇列、自己領、初審兩個人蓋章、重審一個人，任何一個駁回即駁回，
審核頁不顯示作者（盲審）。過審綁著公開指紋（名稱、簡介、封面、標籤、開場白）——作者之後改了
訪客看得到的東西，下一輪同步發現指紋變了就離榜、重新排隊。卡片離開榜單的另一種方式仍然是作者自己撤銷登記。

審核人要看得到作者的完整設定，而那是作者私有的。審核是本站自己的事（owner 2026-09-18），
不要求供應商提供分享介面或服務帳號：作者提交時，本站在那一個請求裡用作者自己的 token 讀一次
整份設定（卡片、開場白、綁定的世界書、作者資產），存成這張審核單的**快照**
（`review_snapshots`，`src/review-snapshot.ts`），審核人看快照。單子一定案（過審或駁回）或作者
撤銷登記，快照就刪——本站不長期保存任何卡片的私有設定，也不持有任何供應商那邊的金鑰。
同步開的重審單沒有快照（那時手上沒有作者的 token），審核人看的是變動後的公開資料。
`REVIEW_ENABLED` 不是 `"true"` 的部署回到「登記即上榜」。
審核人初期由站方手動登記：`node scripts/grant-reviewer.mjs <主站公開數字 ID>`（加 `--revoke` 取消）。

## 對上游的依賴

本站有自己的成員 ID（`members.handle`，8 個小寫字母；作者頁網址、「我的」頁顯示的都是它），供應商帳號是掛在成員底下的身分（`member_identities`），上游的數字 ID 只在那張表裡當映射鍵。登入先到本站的 `/login` 選供應商，再跳去供應商的授權頁。完整的契約在 [docs/developers.md](developers.md)（總覽，英文）與 [docs/openapi.json](openapi.json)（每個端點的參數、請求、回應與錯誤碼，OpenAPI 3.1），站上 `/developers` 頁渲染這兩份；`web/test/protocol-doc.test.ts` 要求程式碼打的每條上游路徑都在規格檔裡。下表是站台伺服器自己會發的那幾條。

社群站是單一產品介面（owner 2026-09-20 裁決）：對話清單、收藏、關注與個人資料由社群成員持有。「我的卡片」跨已連接服務合併作品，積分頁同頁顯示各服務餘額與流水，不提供全站的平台切換。卡片的可玩服務依託管與發布狀態決定，開始遊玩時才選擇服務；播放器憑證限定在該次遊玩，不改變社群登入。社群清單不讀 SaaS 的 `/conversation/list`；具體資料及回訪契約見 [community-library.md](community-library.md)。

服務的角色 ID 與帳號 ID 仍各自有命名空間，API 內部使用 provider 區分請求目標與驗證 issuer；這是路由及權限邊界，不是使用者必須切換的社群模式。`work_copies` 副本不上榜，卡片頁按實際可用副本提供遊玩選項。服務費用各自記帳，不合併餘額。

| 呼叫 | 憑證 | 何時 |
|---|---|---|
| `GET /open/v1/me` | 轉發作者自己的 token | 登記時確認這張卡真是他的；每個需要身分的請求 |
| `GET /open/v1/role/detail` | 無 | 每小時同步，那時沒有使用者在線 |
| `GET /open/v1/role/detail`（作者視角）、`/worldbook/bindings`、`/worldbook/detail`、`/worldbook/entry/list`、`/role/author-asset` | 轉發作者自己的 token | 提交審核時讀一次整份設定，存成審核快照 |

以上是**這個服務自己**發出的呼叫。建卡編輯器另外會用作者的 token 直接打上游
...
本站自己的分級，不讀供應商的（供應商有沒有分級是它的事）。作者提交時必須宣告「一般內容／成人內容」，
宣告記在審核單上；審核人對照內容，不符就駁回（審核人不替作者改宣告）。在榜的卡改了宣告視同內容變了，
要重審。

預設全站不展示成人內容：榜單、搜尋、標籤列、作者榜、作者頁、卡片頁、分享預覽都當它不存在；直接拿到
連結會看到一道門：沒登入的人被要求登入，登入了的人就地確認年齡（一次性）並自動開啟。成員在「使用者設定」開啟才看得到；第一次開要填出生日期，伺服器只算滿不滿 18，
**不保存生日**，只記驗過的時間。開了的請求帶 `?nsfw=1` 加 token，伺服器每次驗證上游身分與本站的即時年齡／開關狀態，再讀取獨立的成人榜單快取；回給瀏覽器仍是 `private, no-store`。
審核人要驗過年齡才能領成人內容的審核單。
...
欄位上限不寫死在前端，由 `GET /open/v1/role/validate` 回來：`customInstructions` 的上限隨卡片語區
...
| 路徑 | 層 | 存什麼 | 存活 |
|---|---|---|---|
| `/v1/cards`、`/v1/authors`、`/v1/tags` | 邊緣 Cache API + KV | 榜單 JSON，依內容分級與查詢條件隔離 | 300s |
...
榜單先讀同節點 Cache API，再讀既有 `CACHE` KV，最後才查 D1 重算清單。一般／成人內容分開，
成人內容每次仍驗上游身分與本站即時設定；會員查詢是一次唯讀 SQL，不在看榜時建立或更新會員。
隨機推薦維持每次重抽，不快取。作者榜和標籤統計維持只算一般內容。

每份快取含絕對到期時間，KV 回填邊緣層不重新延長五分鐘。KV 最終一致，冷節點可能仍須回 D1；
任何快取讀寫故障都回退查詢，不讓快取成為可用性依賴。語言、排序、時間窗、標籤、隱藏類型、作者、
分頁及已驗證的內容分級都進鍵；無關追蹤參數不進鍵，令牌與會員身分不儲存。

每次請求仍即時讀取 D1 的 `moderation_clock`，並將版本放進兩層快取鍵。
審核下架以及 migration `0031` 涵蓋的卡片審核狀態、成人分級、核准版本變更與刪除立即失效；
排名、熱度、新登記內容可晚最多五分鐘反映。暖一般榜單仍有一次 D1 查詢，暖成人榜單有兩次，
不能宣稱零 DB；成人路徑也仍須等待上游身分驗證。對外使用 `no-store`（成人為 `private, no-store`），
確保下次請求會重新驗權限和審核版本。

個人的 `/v1/me/cards` 維持原有 Cache API 設計，不使用這份 KV 榜單快取。

真正保證一致性的不是層數，是把資料按變動頻率切開：慢而穩的（有哪些卡）快取，
快而使用者正在操作的（哪些已登記）永遠即時查。所以就算命中快取，按下登記之後
看到的狀態一定是對的。

私人卡片快取鍵使用**驗證過的供應商＋數字 ID**，絕不用 token 或任何請求帶進來的值——鍵裡混進可偽造
的輸入，就會把一個人的清單送給另一個人。前端在自己剛改過卡之後會帶 `?fresh=1`
繞過所有層：它知道自己寫過，比任何 TTL 都準。

榜單回應帶 `X-Cache: hit|miss`、`X-Cache-Layer: edge|kv|origin`，以及 `Server-Timing`：
`moderation`、`access`（卡片榜）、`cache`、`query`（未命中時）。可用瀏覽器 Network 分辨驗證、
資料庫排序與網路等待。其他私人 API 的 `X-Cache: bypass` 語義維持原樣。
...
服務端只看得到打本站 `/v1/*` 的那些（留言是本站自己的 API，`comment_create` 由服務端記）。**建立與編輯卡片、錢包全部是直接打上游的跨域請求，
...
本專案以 [GNU AGPL 3.0](./LICENSE) 授權。社群自己 fork、自己部署、自己再開源都沒有問題；
拿去架站提供服務（包括商業服務）也可以，但依 AGPL 的規定，你改過的完整原始碼必須同樣公開給
使用者。`stage/` 子模組是上游的獨立專案，維持它自己的授權條款，不受本專案條款約束。

### 社群頁面讀取快取

公開作者資訊採批次讀取與五分鐘內部 Edge／KV 快取，D1 版本檢查保留公開同意、撤銷與到期邊界；冷頁面再用一次批次 SQL 投影。作者統計五分鐘，徽章定義三十分鐘，留言計數三十秒，均有對應版本失效。私人清單只在當前分頁、登入憑證與供應商範圍內快取三十秒，異動與登出會清除。

留言面板首次開啟才載入；同一請求的上游身分驗證合併，library／community 計數寫入移至 `waitUntil`。完整鍵、遷移、失效與驗證決策見 [社群讀取效能](community-cache-performance.md)。
