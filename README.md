...
- 榜單排序 `ORDER BY (talk_num - talk_num_prev)` 是每行現算的，用不上索引；
  `COUNT(*)` 每次請求全表數一遍。卡量到幾萬張時要把分數落成欄位 + 建索引，
  再大就在每輪同步後把前 N 名預算好寫進 KV（榜單是公開資料，KV 的最終一致沒有影響），
  讀取變成一次 KV get。要做到「登記完榜單立刻更新」則需要一個全域強一致的代際計數器
  （Durable Object），把代際號拼進快取鍵。
